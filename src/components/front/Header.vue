...
const drawerProductCategory = useTemplateRef<InstanceType<typeof DrawerProductCategory>>("drawerProductCategory");
  ...
  <nav class="navbar">
    <div class="top">
      ...
      <div class="products all" v-if="route.path !== '/products'">
        <RouterLink class="products nav-link" to="/products"
          ><h6>全部產品</h6></RouterLink
        >
      </div>
      <div class="brand">
      ...
      <div class="cart">
        ...
        <template v-else-if="!isMobile && isProductPage">
          <ElPopover popper-class="cart-popover" placement="bottom" width="300">
            <template #reference>
              <ElBadge
                :value="cartStore.cartList.length"
                :max="99"
            ...
            <template v-if="cartStore.cartList.length !== 0">
              <ElButton type="primary" @click="router.push('/cart')"
                >購物車結帳</ElButton
              >
            </template>
          </ElPopover>
    ...
    <div class="category" v-if="route.path === '/products'">
      <div class="products all">
        <router-link class="products nav-link" to="/products"
          ><h6>全部產品</h6></router-link
        >
      </div>
      <div class="products earrings">
        <router-link
          class="products nav-link"
          :to="{ path: '/products', query: { category: '耳環' } }"
        >
          <h6>耳環</h6>
        </router-link>
      </div>
      <div class="products ring">
        <router-link
          class="products nav-link"
          :to="{ path: '/products', query: { category: '戒指' } }"
        >
          <h6>戒指</h6>
        </router-link>
      </div>
      <div class="products necklace">
        <router-link
          class="products nav-link"
          :to="{ path: '/products', query: { category: '項鍊' } }"
        >
          <h6>項鍊</h6>
        </router-link>
      </div>
      <div class="products bracelet">
        <router-link
          class="products nav-link"
          :to="{ path: '/products', query: { category: '手鐲' } }"
        >
          <h6>手鐲</h6>
        </router-link>
      </div>
