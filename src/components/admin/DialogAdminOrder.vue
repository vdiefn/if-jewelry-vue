...
import type { OrderData,EditOrderParams } from "@/types/admin/order";

const dialogVisible = ref(false);
const loading = ref(false);
const emit = defineEmits(["order-update"]);
const initialForm: EditOrderParams = {
  id:"",
  create_at: 0,
  is_paid: false,
  paid_date: undefined,
...
const open = (row: OrderData) => {
  Object.assign(form, initialForm);
  Object.assign(form, {
    ...row,
    create_at: row.create_at * 1000,
    paid_date: row.paid_date ? row.paid_date * 1000 : undefined,
  });
...
const confirm = async () => {
  ...
  const payload = {
    ...
    paid_date: form.create_at? Number(form.paid_date) / 1000 : undefined,
  };
