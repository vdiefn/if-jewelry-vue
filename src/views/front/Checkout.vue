...
type DeliveryType = "store pickup" | "home delivery"
type PaymentType = "atm transfer" | "credit card"
interface RuleForm {
  name: string;
  address: string;
  phone: string;
...
const validatePickupStore = (_rule:any, value:any, callback:any) => {
...
const validateCardNumber = (_rule:any, value:any, callback:any) => {
  if (!value) {
...
const validateCvvNumber = (_rule:any, value:any, callback:any) => {
  if (!value) {
...
const creditCardRules = reactive<FormRules<CreditCardForm>>({
  cardHolderName: [
    { required: true, message: "請填寫持卡人姓名", trigger: "blur" },
  ],
          ...
          <ElForm :model="form" label-width="auto" :rules="rules" ref="formRef">
            <ElFormItem label="寄送方式" prop="delivery">
              <ElRadioGroup v-model="form.delivery">
                ...
                <ElRadio label="超商取貨不付款" value="store pickup"
                  >全家取貨不付款</ElRadio
                >
              </ElRadioGroup>
            </ElFormItem>
            <ElFormItem
              label="超商店名"
              prop="pickupStore"
              v-if="form.delivery === 'store pickup'"
            >
          ...
          <ElForm label-width="100px" :model="form" :rules="rules">
            ...
            <template v-if="form.payment === 'credit card'">
              <ElForm
                ...
                label-width="100px"
                class="credit-card-info-wrapper"
                ref="creditCardFormRef"
                :rules="creditCardRules"
              >
                <ElFormItem
                  label="持卡人姓名"
                  class="card-holder-wrapper"
                  prop="cardHolderName"
                >
                  <ElInput
                ...
                <ElFormItem label="信用卡卡號" class="credit-card-grid-wrapper">
                  <div class="credit-card-grid">
                    <ElFormItem prop="cardNumber1" label-width="0">
                      <ElInput
                        v-model="creditCardForm.cardNumber1"
                        placeholder="1234"
                        maxlength="4"
                      />
                    </ElFormItem>
                    <ElFormItem prop="cardNumber2" label-width="0">
                      <ElInput
                        v-model="creditCardForm.cardNumber2"
                        placeholder="5678"
                        maxlength="4"
                      />
                    </ElFormItem>
                    <ElFormItem prop="cardNumber3" label-width="0">
                      <ElInput
                        v-model="creditCardForm.cardNumber3"
                        placeholder="9012"
                        maxlength="4"
                      />
                    </ElFormItem>
                    <ElFormItem prop="cardNumber4" label-width="0">
                      <ElInput
                        v-model="creditCardForm.cardNumber4"
                        placeholder="3456"
                        maxlength="4"
                      />
                    </ElFormItem>
                ...
                <ElFormItem label="後三碼" class="cvv-wrapper" prop="cvvNumber">
                  <ElInput
                    v-model="creditCardForm.cvvNumber"
                    placeholder="後三碼"
                    maxlength="3"
                    show-password
                  />
      ...
      <ElCollapse>
        <ElCollapseItem class="order-info-container">
          ...
          <div class="final-info">
            <p>
              使用優惠券:
              {{ cartStore.couponCode ? cartStore.couponCode : " 無 " }}
            </p>
