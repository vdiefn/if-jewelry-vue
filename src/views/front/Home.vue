...
<template>
  <div class="carousel-container" v-if="data">
    <ElCarousel type="card">
      <ElCarouselItem
        v-for="product in data.slice(0, 4)"
        :key="product.id"
        class="carousel-item"
      >
        <RouterLink :to="{ path: `/product/${product.id}` }">
          <img
            :src="product.imagesUrl[0] ?? noImage"
  ...
  <div class="category-container">
    <template v-if="earringsProducts.length > 0">
      ...
      <div class="earrings card-wrapper">
        <CardProduct
          :data="product"
          v-for="product in earringsProducts"
          :key="product.id"
        />
      </div>
    </template>
    ...
    <template v-if="necklaceProducts.length > 0">
      ...
      <div class="necklace card-wrapper">
        <CardProduct
          :data="product"
          v-for="product in necklaceProducts"
          :key="product.id"
        />
      </div>
    </template>
    ...
    <template v-if="braceletProducts.length > 0">
      ...
      <div class="bracelet card-wrapper">
        <CardProduct
          :data="product"
          v-for="product in braceletProducts"
          :key="product.id"
        />
      </div>
    </template>
